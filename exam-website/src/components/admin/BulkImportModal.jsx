...
export default function BulkImportModal({ isOpen, onClose, categoryId, currentUserId, onSuccess }) {
  ...
  // เรียกใช้ฟังก์ชันเปิดป็อปอัปแจ้งเตือนจากระบบกลาง
...
(ใช้เครื่องหมาย --- คั่นระหว่างข้อเสมอ)`
  ...
  const handleCopyPrompt = async () => {
    try {
      ...
      showModal({
        ...
        description: 'เบราว์เซอร์ของคุณไม่รองรับการคัดลอกอัตโนมัติ กรุณาคลุมข้อความและกดก๊อปปี้ด้วยตัวเองครับ'
      })
    }
  }

  // 🌟 ฟังก์ชันประมวลผลข้อความ (รับค่า e เพื่อดักไม่ให้ฟอร์มรีเฟรชหน้าเว็บ)
  const handleParseBulkText = (e) => {
    if (e) e.preventDefault() // ป้องกันการรีเฟรชหน้าเว็บจากคำสั่ง Submit
    
    // ไม่ต้องมี if (!bulkInputText.trim()) แล้ว เพราะ HTML5 required ในกล่อง textarea ดักให้เราแล้วครับ
    ...
    if (newParsedList.length === 0) {
      return showModal({
        type: 'danger',
        theme: 'dark',
        title: 'ฟอร์แมตข้อความไม่ถูกต้อง',
        description: 'ไม่พบข้อมูลข้อสอบ กรุณาตรวจสอบว่ามีเครื่องหมาย --- คั่นระหว่างข้อและมีตัวเลือก A. B. C. D. ครบถ้วนครับ'
      })
    }
  ...
  const handleConfirmBulkSave = async (asActive = true) => {
    if (!categoryId) {
      return showModal({
        ...
        description: 'กรุณาเลือกหมวดหมู่วิชาสอบที่ต้องการนำเข้าก่อนครับ'
      })
    }
    setSaving(true)
    ...
    try {
      ...
      showModal({
        type: 'success',
        theme: 'dark',
        title: '🎉 นำเข้าสำเร็จเรียบร้อย!',
        description: `บันทึกข้อสอบจำนวน ${parsedQuestions.length} ข้อในสถานะ "${statusText}" เรียบร้อยแล้วครับ`
      })
      ...
      showModal({
        ...
        description: 'เกิดข้อผิดพลาดในการบันทึกข้อมูล: ' + err.message
      })
    } finally {
      setSaving(false)
    }
  ...
  return (
    /* 🌟 ดัน z-index เป็น !z-[999] บังคับลอยเหนือ Navbar 100% */
    <div className="fixed inset-0 !z-[999] overflow-hidden">
      {/* Backdrop มืด */}
      <div 
        onClick={handleClose} 
        className="absolute inset-0 bg-slate-950/80 backdrop-blur-xs !z-[999] transition-opacity animate-fadeIn cursor-pointer"
      />
      
      {/* Slide-over Panel กว้างพิเศษ (Off-Canvas Drawer) */}
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-6 sm:pl-10 !z-[1000]">
        <div className="w-screen max-w-4xl bg-slate-900 border-l border-slate-800 shadow-2xl overflow-y-auto flex flex-col relative animate-slideLeft">
          ...
          <div className="p-6 flex-1 flex flex-col gap-6">
            {bulkStep === 'input' && (
              <>
                ...
                {/* 🌟 ครอบช่องพิมพ์และปุ่มด้วย <form> เพื่อให้เบราว์เซอร์แจ้งเตือนช่องว่างอัตโนมัติ */}
                <form onSubmit={handleParseBulkText} className="flex-1 flex flex-col gap-4">
                  <div className="flex-1 flex flex-col gap-2">
                    <h3 className="font-bold text-slate-300 text-sm">2. วางผลลัพธ์ข้อความที่ได้จาก AI ลงในกล่องนี้:</h3>
                    <textarea
                      required // 🌟 สั่งให้เบราว์เซอร์แจ้งเตือนเมื่อช่องนี้ว่างเปล่า (Please fill out this field)
                      value={bulkInputText}
                  ...
                  <div className="pt-2 border-t border-slate-800 flex justify-end">
                    <button 
                      type="submit" // 🌟 เปลี่ยนเป็นปุ่ม Submit เพื่อเรียกใช้งาน Form Validation
